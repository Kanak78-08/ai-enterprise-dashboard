...
const COLLECTIONS = new Set(["users", "reports", "analytics", "activities"]);
...
const handleCollectionRoute = async (req, res, url) => {
  ...
  if ((req.method === "PATCH" || req.method === "PUT") && id) {
    ...
    const item = req.method === "PUT" ? { id, ...body } : { ...items[index], ...body };
    db[collectionName][index] = item;
    writeDb(db);
  ...
  if (req.method === "DELETE" && id) {
    ...
    if (nextItems.length === items.length) return sendJson(res, 404, { error: "Item not found." });
    db[collectionName] = nextItems;
    writeDb(db);
